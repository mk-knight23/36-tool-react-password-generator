...
const shortcutsList = [
  { action: 'Generate Password', keys: KEYBOARD_SHORTCUTS.GENERATE },
  { action: 'Copy Password', keys: KEYBOARD_SHORTCUTS.COPY },
  { action: 'Toggle Visibility', keys: KEYBOARD_SHORTCUTS.TOGGLE_VISIBILITY },
...
function exportSettings() {
  ...
  a.href = url
  a.download = 'vaultpass-data.json'
  a.click()
        ...
        <div class="relative bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
          <div class="flex items-center justify-between px-8 py-6 border-b border-slate-800">
            <h2 class="text-xl font-display font-black uppercase tracking-tight text-white flex items-center gap-3">
              <Settings :size="24" class="text-vault-primary" />
              Settings
            </h2>
          ...
          <div class="p-8 space-y-8 max-h-[60vh] overflow-y-auto">
            <div class="space-y-6">
              ...
              <div class="flex items-center justify-between p-4 bg-slate-800/50 rounded-2xl">
                <div class="flex items-center gap-4">
                  <div class="p-3 bg-vault-primary/20 rounded-xl">
            ...
            <div class="space-y-6">
              ...
              <div class="flex items-center justify-between p-4 bg-slate-800/50 rounded-2xl">
                <div class="flex items-center gap-4">
                  <div class="p-3 bg-vault-primary/20 rounded-xl">
                    <Volume2 v-if="settingsStore.settings.soundEnabled" :size="20" class="text-vault-primary" />
                ...
                <button
                  ...
                  :class="settingsStore.settings.soundEnabled ? 'bg-vault-primary' : 'bg-slate-600'"
                >
                  <div
            ...
            <div class="space-y-6">
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div class="p-4 bg-slate-800/50 rounded-2xl text-center">
                  <p class="text-2xl font-black text-white">{{ statsStore.stats.passwordsGenerated }}</p>
                  <p class="text-xs text-slate-500 uppercase tracking-wider">Generated</p>
                </div>
                <div class="p-4 bg-slate-800/50 rounded-2xl text-center">
          ...
          <div class="px-8 py-6 border-t border-slate-800 bg-slate-800/30 flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <button @click="close" class="px-6 py-2 bg-vault-primary hover:bg-vault-accent text-white rounded-xl text-sm font-bold shadow-lg transition-all active:scale-95">
                Done
              </button>
            </div>
          </div>
